...
internal sealed class RedisMessageConverter : IRedisMessageConverter
{
    ...
    private static object? DetermineMessageSubType(string? messageSubtype, ReadOnlySpan<byte> data)
    {
        return messageSubtype switch
        {
            ...
            { } when String.Equals(messageSubtype, nameof(PlayerListMemberWhiteCardMessage), StringComparison.OrdinalIgnoreCase) => ReadObject<PlayerListMemberWhiteCardMessage>(data),
    ...
    private static Task WriteToStreamAsync(object data, Stream stream, CancellationToken cancellationToken = default) => JsonSerializer.SerializeAsync(stream, data, JsonSerializerOptions, cancellationToken);
    ...
    private static ValueTask<T?> ReadObjectAsync<T>(Stream data, CancellationToken cancellationToken = default) => JsonSerializer.DeserializeAsync<T>(data, JsonSerializerOptions, cancellationToken);
    private static async Task<byte[]> ReadBinaryAsync<T>(byte[] data, CancellationToken cancellationToken = default)
    {
        await using var dataStream = new MemoryStream(data);

        await using var deflateStream = new DeflateStream(dataStream, CompressionMode.Decompress);

        await using var memoryStream = new MemoryStream();

        await deflateStream.CopyToAsync(memoryStream, cancellationToken);
